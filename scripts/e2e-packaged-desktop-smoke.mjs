...
const OUTPUT_DIR = path.join(ROOT_DIR, "output", "playwright", "e2e-packaged-desktop-smoke");
const RELEASE_DIR = path.join(ROOT_DIR, "output", "releases", "desktop");
const API_URL = process.env.E2E_API_URL ?? "http://127.0.0.1:4000";

let startedApiProcess = null;

function log(message) {
  process.stdout.write(`[packaged-desktop-e2e] ${message}\n`);
}
...
async function runCommand(command, args, cwd = ROOT_DIR, extraEnv = {}) {
...
async function ensurePackagedArtifact() {
  if (process.platform !== "darwin") {
    throw new Error("Packaged desktop smoke currently supports macOS artifacts only.");
  }

  log("Packaging desktop app");
  await runCommand("pnpm", ["package:desktop"]);

  const entries = await readdir(RELEASE_DIR, {
    withFileTypes: true
  });
  const zipArtifact = entries
  ...
  if (!zipArtifact) {
    throw new Error(`No macOS desktop zip artifact found in ${RELEASE_DIR}.`);
  }

  return path.join(RELEASE_DIR, zipArtifact);
}

async function unpackArtifact(zipArtifactPath) {
  const unpackDir = path.join(OUTPUT_DIR, "unpacked");
  ...
  log(`Unpacking ${path.basename(zipArtifactPath)}`);
  await runCommand("ditto", ["-x", "-k", zipArtifactPath, unpackDir]);

  const appName = "OpenClaw Team OS.app";
  const appBundlePath = path.join(unpackDir, appName);
  const executablePath = path.join(appBundlePath, "Contents", "MacOS", "OpenClaw Team OS");
...
async function runPackagedDesktopFlow(executablePath, artifactPath) {
  const screenshots = [];
  const consoleErrors = [];
  const pageErrors = [];
  ...
  try {
    ...
    const organizationName = `Packaged Smoke Org ${runId}`;
    ...
    log("Creating organization in packaged app");
    await page.getByRole("button", { name: /新组织/ }).click();
    await page.getByLabel("组织名称").fill(organizationName);
    await page.getByLabel("管理员名称").fill("Packaged Founder");
    await page.getByLabel("管理员邮箱").fill(`packaged+${runId}@example.com`);
    ...
    log("Recruiting first bot team in packaged app");
    ...
    log("Switching to team workspace in packaged app");
    await page.getByRole("button", { name: /^团队$/ }).click();
    await page.getByRole("heading", { name: "团队", exact: true }).waitFor({
      timeout: 15000
    });
    ...
    log("Submitting first task in packaged app");
    ...
    await page
      .getByLabel("一句话目标")
      .fill("请为一款两人团队可做的像素风 Roguelike 游戏输出首周制作计划与首版原型建议。");
    ...
    await page.getByText(`${hiredUnitName} 任务`).first().waitFor({
      timeout: 15000
    });
    ...
    log("Approving task in packaged app");
    await page.getByRole("button", { name: "去审批" }).click();
    await page.getByRole("heading", { name: "审批", exact: true }).waitFor({
      timeout: 15000
    });
    await page.getByText(`审批 ${hiredUnitName} 任务`).waitFor({
      timeout: 15000
    });
    ...
    log("Checking deliverables in packaged app");
    await page.getByRole("button", { name: /^任务台$/ }).click();
    await page.getByRole("heading", { name: "任务台", exact: true }).waitFor({
      timeout: 15000
    });
    await page.getByRole("heading", { name: `${hiredUnitName} 任务 交付包`, exact: true }).waitFor({
      timeout: 15000
    });
    await page
    ...
    if (unexpectedConsoleErrors.length > 0 || pageErrors.length > 0) {
      throw new Error(
        [
          "Unexpected packaged Electron renderer errors detected.",
    ...
    try {
      ...
      // Preserve the original packaged-app failure when the window can no longer be captured.
    }
    throw error;
  } finally {
...
async function main() {
  ...
  const artifactPath = await ensurePackagedArtifact();
  const { executablePath } = await unpackArtifact(artifactPath);
  await ensureApi();

  const summary = await runPackagedDesktopFlow(executablePath, artifactPath);
  const summaryPath = path.join(OUTPUT_DIR, "summary.json");
  await writeFile(summaryPath, `${JSON.stringify(summary, null, 2)}\n`, "utf8");
  log(`Packaged desktop smoke flow passed. Summary saved to ${summaryPath}`);
}
